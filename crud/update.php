...
if (isset($_GET['idfilm'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt = $pdo->prepare('
            UPDATE Film 
            SET idfilm = :idfilm, titre = :titre, annee = :annee, idGenre = :idGenre, resume = :resume 
            WHERE idfilm = :original_id
        ');
        $stmt->execute([
            ':idfilm' => $idfilm,
            ':titre' => $titre,
    ...
    $stmt = $pdo->prepare('
        SELECT Film.idfilm, Film.titre, Film.annee, Film.idGenre, Genre.nomGenre, Film.resume 
        FROM Film 
...
$stmt_genre = $pdo->query('SELECT * FROM Genre ORDER BY nomGenre');
